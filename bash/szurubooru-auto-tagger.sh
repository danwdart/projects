...
tag_next_batch() {
    ...
    echo $RESP | jq -r ".results[]|[.id, .checksumMD5, .contentUrl, .version] | @tsv" |
        while IFS=$'\t' read -r id MD5 contentURL version; do
            ...
            echo "Hitting $URL..."
            CACHE_FILE=".jsoncache/$MD5.json" 
            if [[ ! -f $CACHE_FILE ]]
            then
                echo "debug: Danbooru Cache Miss"
                curl -u "$DB_USER:$DB_API_KEY" -s "$URL" > $CACHE_FILE 2>/dev/null
                if [[ ! -f $CACHE_FILE ]]
                then
            ...
            then
                ...
                BODY=$(echo -n "{\"version\":$version,\"tags\":$(echo -n "$TAGS $TAG_TAGGED" | jq -sRc 'split(" ")'),\"source\":\"$DB_SOURCE\"}")
            fi
