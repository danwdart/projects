...
const sentences = text.match(/([A-Z].+\.)/g), chooseRandom = (arr) => arr[Math.floor(Math.random() * arr.length)], pickByProbability = (map) => {
    const rand = Math.random(), total = Array.from(map.values()).reduce((a, b) => a + b), toExceed = rand / total;
    let soFar = 0;
    ...
    return '';
}, byFreq = new Map(), firsts = sentences.map(sentence => sentence.split(/\s+/)[0]), lasts = sentences.map(sentence => {
...
let ended = false, word = chooseRandom(firsts);
while (!ended) {
