# Interesting Haskell things

## Requirements

async

## Links
https://wiki.haskell.org/The_JavaScript_Problem#Haskell_-.3E_JS
https://wiki.haskell.org/Applications_and_libraries/Operating_system
http://programatica.cs.pdx.edu/House/
http://repetae.net/computer/jhc/
https://www.stackbuilders.com/news/concurrent-haskell-in-the-real-world
https://hackage.haskell.org/package/mongoDB-2.4.0.1/docs/Database-MongoDB-Query.html#t:Action
https://hackage.haskell.org/package/base-4.12.0.0/docs/Data-Maybe.html
https://en.m.wikibooks.org/wiki/Haskell/Control_structures
https://github.com/neurocyte/android-haskell-activity
https://wiki.haskell.org/Frag
https://hackage.haskell.org/package/aeson
https://hackage.haskell.org/package/req
https://wiki.haskell.org/OpenGL
https://wiki.haskell.org/Applications_and_libraries/GUI_libraries
https://wiki.haskell.org/X_window_programming_in_Haskell
https://hackage.haskell.org/package/concurrent-extra-0.7.0.12/docs/Control-Concurrent-Event.html
http://book.realworldhaskell.org/read/using-databases.html
https://en.wikibooks.org/wiki/Haskell/Control_structures
https://hackage.haskell.org/package/async-2.2.1/docs/Control-Concurrent-Async.html
https://wiki.haskell.org/Applications_and_libraries/Operating_system
https://wiki.haskell.org/Real_World_Applications/Event_Driven_Applications

