...
import Discord.Types  (ChannelId, Message, Event (..), messageText, userName, messageAuthor)
...
handleMessage h username mt = case T.unpack mt of
...
handleEvent h e = case e of
    ...
    _ -> Prelude.putStrLn "Event detected. Not handled."

handleQuit :: IO ()
handleQuit = Prelude.putStrLn "Quit handler called"
...
main = do
    _ <- runExceptT (
        catchE ( do
            ...
            _ <- liftIO . runDiscord . runDiscordOpts . T.pack $ token
